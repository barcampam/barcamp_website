...
Tipo di avviso di informazioni sull'icona
...
Notifica
Di base
L'utilizzo più semplice che chiude la casella di notifica dopo 4.5s.
...
Notifica con icona
Una casella di notifica con un'icona sul lato sinistro.
...
Sono disponibili tre dimensioni di una casella Input: grande (42px), predefinito (35px) e piccolo (30px). Nota: All'interno delle forme viene utilizzata solo la grande dimensione.
Gruppo di input
Esempio di input.Group Nota: Non è necessario Col per controllare la larghezza nella modalità compatta.
Autosizing l'altezza per adattarsi al contenuto
autosize prop per un tipo di textarea dell'input rende l'altezza regolabile automaticamente in base al contenuto. Può essere fornito un oggetto opzioni per autosizzare per specificare il numero minimo e massimo di righe che l'area textarea regolerà automaticamente.
Scheda Pre / Post
...
Radio
Radio di base
L'uso più semplice. Usare disabilitato per disattivare una radio.
...
Carte
Scheda di base
Una scheda di base contenente un titolo, un contenuto e un contenuto aggiuntivo d'angolo.
Di Più
Titolo della carta
Contenuto della scheda
Il peso del peso è ridotto, l'elit di adipisizione del consectetur, che rende meno efficace il lavoro e la dolce magna aliqua. L'ut enim ad minim veniam, quis nostrud esercizio ullamco laboris nisi ut aliquip ex ea commodo consequat.
Nessun bordo
...
Questa è l'intestazione del pannello 1
Questa è l'intestazione del pannello 2
Questa è l'intestazione del pannello 3
Questo è il pannello nido del pannello
Esempio nidificato
Il crollo è nidificato all'interno del Collapse.
Esempio senza bordi
...
popover
Esempio di base
L'esempio più semplice. La dimensione dello strato galleggiante dipende dalla regione dei contenuti.
Allontanami
Titolo
...
La generazione di un insieme di tag per array consente di aggiungere e rimuovere in modo dinamico. Il suo è basato sull'evento afterClose, che verrà attivato mentre la fine animazione fine.
+ Nuovo tag
Sequenza temporale
Esempio di base
Timeline di base
...
Vedi altro
costume
Imposta un nodo come un'icona o un altro elemento personalizzato.
Esempio di colore
...
Altro carattere
Sostituire la stella predefinita in altri caratteri come l'alfabeto, la cifra, l'icona o anche la parola cinese.
Albero
Esempio di base
L'utilizzo più semplice, ti dice come utilizzare controllibili, selezionabili, disattivati, defaultExpandKeys e così via.
Esempio controllato di base
esempio controllato di base
Esempio Draggable
Trascinare alberoNode da inserire dopo l'altro alberoNodo o inserire nell'altro albero TreeNode.
...
isomorfo
Sono d'accordo con i termini e le condivisioni
Registrazione
...
Mappa di base (con il simbolo personalizzato dell'icona)
